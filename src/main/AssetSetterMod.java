...
public class AssetSetterMod {
    GamePanel gp;
    TileManager tileM;

    public AssetSetterMod(GamePanel gp, TileManager tileM) {
        this.gp = gp;
        this.tileM = tileM;
    }
    ...
    public ArrayList<Integer> availablePos(){
        ...
        for(int i=0; i<10; i++){  // 13 è il massimo dei tiles della mappa per la x
            for(int j=0; j<13; j++){  // 10 è il massimo dei tiles della mappa per la y
                if(tileM.blockTileNum[i][j] != 3 && tileM.blockTileNum[i][j] != -1){  // se non è un palazzo o non è gia preso
                    if(true)  // pos vicino alla partenza del player 
                        avPos.add( j * 10 + i); // calcola l'indice unico per la posizione
                }
            }
        }
        return avPos;
